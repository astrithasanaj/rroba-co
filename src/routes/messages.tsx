      ...
      >
        ...
        >
          {mode === "archive" ? (
            <button
              type="button"
              onClick={() => navigate({ to: "/messages", search: (prev) => ({ ...prev, view: "list", tab }) })}
              aria-label="Kthehu"
          ...
          {mode === "inbox" ? (
            <div className="flex items-center gap-1 rounded-full px-2 py-1.5" style={{ backgroundColor: "#2d1521" }}>
              <button onClick={() => navigate({ to: "/messages", search: (prev) => ({ ...prev, view: "archive", tab }) })} aria-label="Arkiva" className="grid h-11 w-11 place-items-center rounded-full transition-transform active:scale-90">
                <InboxIcon />
              </button>
              <button onClick={() => navigate({ to: "/messages", search: (prev) => ({ ...prev, view: "new", tab }) })} aria-label="Mesazh i ri" className="grid h-11 w-11 place-items-center rounded-full transition-transform active:scale-90">
                <ComposeIcon />
              </button>
            </div>
          ) : <div className="w-10" />}
            ...
            >
              ...
              {(["all", "buy", "sell"] as const).map((t) => {
                ...
                return (
                  <button
                    key={t}
                    onClick={() => navigate({ to: "/messages", search: (prev) => ({ ...prev, tab: t, view: "list" }) })}
              ...
              {filtered.map((t) => (
                <li key={t.id} className="relative overflow-hidden" style={{ borderBottom: `1px solid ${DIVIDER}`, minHeight: 72 }}>
                  ...
                  <div
                    ...
                    onClick={() => { if (swipeId === t.id) { setSwipeId(null); return; } navigate({ to: "/messages", search: (prev) => ({ ...prev, thread: t.id }) }); }}
                  >
...
function NewMessage({ me }: { me: string }) {
  ...
  const startChat = async (otherId: string) => {
    ...
    if (existing && existing.length > 0) {
      navigate({ to: "/messages", search: (prev) => ({ ...prev, thread: existing[0].id }) });
      return;
    }
    ...
    <MobileShell fixed>
      ...
      >
        <header className="sticky top-0 z-30 flex items-center gap-3 px-5 pt-5 pb-3" style={{ backgroundColor: `${CREAM}f2` }}>
          <button onClick={() => navigate({ to: "/messages", search: (prev) => ({ ...prev, view: "list", tab: "all" }) })} className="grid h-10 w-10 place-items-center rounded-full" style={{ backgroundColor: CREAM_ALT }}>
