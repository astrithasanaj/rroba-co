...
function ConversationList({ me, mode, tab }: { me: string; mode: "inbox" | "archive"; tab: "all" | "buy" | "sell" }) {
  ...
  const load = async () => {
    ...
    const [profs, listings, lastMsgs] = await Promise.all([
      (supabase as any).from("public_profiles").select("id,name,avatar_url").in("id", otherIds),
...
function NewMessage({ me }: { me: string }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (others.length) {
        const { data: profs } = await (supabase as any).from("public_profiles").select("id,name,avatar_url").in("id", others);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [prof, listing, msgRes] = await Promise.all([
        (supabase as any).from("public_profiles").select("name,avatar_url").eq("id", otherId).maybeSingle(),
